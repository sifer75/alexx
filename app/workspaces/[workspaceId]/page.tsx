...
export default function WorkspaceId({
  ...
  const getKanbans = async (workspaceId: string) => {
    try {
      const response = await fetch(`/api/kanban?workspaceId=${workspaceId}`, {
        method: "GET",
  ...
  const deleteKanban = async (kanbanId: string) => {
    try {
      ...
      return new Response(
        JSON.stringify({ error: "Erreur lors de la suppression du kanban" }),
        {
          headers: { "Content-Type": "application/json" },
          status: 500,
        }
